/**
 * Batch-Processing mit Python-Parsern
 * Nutzt die bewährten Python-Skripte für präzises PDF-Parsing
 */
...
async function main() {
  ...
  console.log('🐍 Batch-Processing mit Python-Parsern\n');
  ...
  // Lade Kreditoren für Mapping
  const kreditoren = await kreditorenCol.find({}).toArray();
  console.log(`📋 ${kreditoren.length} Kreditoren geladen\n`);
  
  // Hole Emails mit PDFs die noch nicht verarbeitet sind ODER alte Rechnungen ohne Betrag
  const toProcess = await inboxCol.find({ 
    pdfBase64: { $exists: true },
    status: 'pending'
  }).limit(batchSize).toArray();
  
  console.log(`📄 ${toProcess.length} PDFs zum Verarbeiten gefunden\n`);
  ...
  for (let i = 0; i < toProcess.length; i++) {
    ...
    try {
      // Rufe Python-Parser auf
      const parsed = await callPythonParser(email.pdfBase64, email.filename);
      ...
      if (!dryRun) {
        ...
        // Prüfe ob schon existiert (basierend auf sourceEmailId)
