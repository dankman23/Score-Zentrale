  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        SUM(CASE WHEN ${feeFilter} THEN ${netTotalExpr} ELSE 0 END) AS platform_fees
      FROM ${orderTable} o
    ...
    return NextResponse.json({
      ...
      platform_fees: parseFloat(row.platform_fees || 0).toFixed(2)
    })
  } catch (error: any) {
    console.error('[/api/jtl/sales/kpi/with_platform_fees] Error:', error)
