...
/**
 * GET /api/jtl/sales/manufacturers-timeseries
 * Zeitreihen-Daten für Hersteller-Umsätze
 */
  ...
  try {
    ...
    if (manufacturers.length === 0) {
      return NextResponse.json({
        ok: false,
        error: 'Keine Hersteller ausgewählt'
      }, { status: 400 })
    }
    ...
    // WHERE-Klausel für Hersteller
    const manufacturerConditions = manufacturers.map((_, idx) => {
      return `LOWER(h.cName) LIKE '%' + LOWER(@manufacturer${idx}) + '%'`
    }).join(' OR ')

    const query = `
      SELECT 
        ${groupByExpr} AS period,
        h.cName AS manufacturer_name,
      ...
      WHERE CAST(o.dErstellt AS DATE) BETWEEN @from AND @to
        ...
        ${orderTypeFilter}
        AND ${articleFilter}
        AND h.kHersteller IS NOT NULL
        AND (${manufacturerConditions})
      GROUP BY ${groupByExpr}, h.cName
      ORDER BY period ASC, manufacturer_name ASC
    `
    ...
    manufacturers.forEach((name, idx) => {
      sqlRequest.input(`manufacturer${idx}`, sql.NVarChar, name)
    })
    ...
    console.error('[/api/jtl/sales/manufacturers-timeseries] Error:', error)
    return NextResponse.json({ 
      ok: false, 
      error: error.message 
    }, { status: 500 })
