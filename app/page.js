...
export default function App() {
  ...
  const [salesFiltersAvailableHersteller, setSalesFiltersAvailableHersteller] = useState([])
  const [salesFiltersAvailableWarengruppen, setSalesFiltersAvailableWarengruppen] = useState([])
  ...
  // Show loading only briefly - auth check runs in background
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const [k1, k2, t1, t2, osRaw, poRaw, expRaw, marginRaw, topPlatRaw, topManufRaw, topSuppRaw] = await Promise.all([
        ...
        getJson(`/api/jtl/sales/top-suppliers?from=${from}&to=${to}&limit=100`)
      ])
      ...
      setTopSuppliers(topSuppRaw?.ok ? topSuppRaw.suppliers : [])
                        ...
                        <ol className="mb-0 small">
                          ...
                          <li>Um die <strong>Chat ID</strong> zu erhalten:
                            <ul>
                              ...
                              <li>Suchen Sie nach <code>chat → id</code> - dort steht die Chat ID</li>
                            </ul>
                          </li>
                        </ol>
                      </div>
